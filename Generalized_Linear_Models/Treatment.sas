...
data Resp;
input  Center $ ID  Treatment $ Sex $ Age Baseline Resp1  Resp2  Resp3  Resp4 ;
cards;
...
proc transpose data=Resp out=Resp1; 
      by Center ID Treatment Sex  Age Baseline  ; 
      var Resp1-Resp4; 
   run; 

Data Resp1; set Resp1; 
IDnew = ID;
rename Col1 = Resp; 
visit = 1;
if _NAME_ = "Resp2" then visit =2;
if _NAME_ = "Resp3" then visit =3;
if _NAME_ = "Resp4" then visit =4;
if Center = "2" then IDnew = ID + 100; 
obs = _N_;
run; quit;
...
proc genmod data=Resp1 descending;
class IDnew Center Treatment Sex Baseline visit;
model Resp = Center Treatment Sex Age Baseline visit /dist=binomial link=logit;
repeated subject=IDnew/type=exch covb;
estimate 'Active vs Placebo' Treatment 1 -1 /exp;
run;

proc genmod data=Resp1 descending;
class IDnew Center Treatment Sex;
model Resp = Center Treatment Sex Age Baseline visit /dist=binomial link=logit;
repeated subject=IDnew/type=exch covb;
estimate 'Active vs Placebo' Treatment 1 -1 /exp;
estimate 'Placebo vs Active' Treatment -1 1 /exp;
run;

proc genmod data=Resp1 descending;
class obs Center Treatment Sex;
model Resp = Center Treatment Sex Age Baseline visit /dist=binomial link=logit;
repeated subject=obs/type=exch covb;
estimate 'Active vs Placebo' Treatment 1 -1 /exp;
run;
